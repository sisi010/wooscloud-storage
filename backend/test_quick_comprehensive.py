"""
WoosCloud Storage - Quick Comprehensive API Test
Tests all major endpoints to verify system integrity
"""
...
print("="*80)
print("  🧪 WOOSCLOUD QUICK COMPREHENSIVE TEST")
print("="*80)
...
try:
    ...
    response = requests.post(
        ...
        headers=headers,
        json={"collection": "quick_test", "data": {"name": "Test", "value": 123}}
    )
    ...
    # List
    response = requests.get(
        f"{BASE_URL}/storage/list?collection=quick_test",
        headers=headers
    )
...
try:
    response = requests.get(
        f"{BASE_URL}/search?query=test&collection=quick_test",
        headers=headers
    )
    test("Full-text search", response.status_code == 200)
    
    response = requests.get(
        f"{BASE_URL}/search/autocomplete?query=te",
        headers=headers
    )
    test("Autocomplete", response.status_code == 200)
...
try:
    response = requests.get(
        f"{BASE_URL}/export/json?collection=quick_test",
        headers=headers
    )
    test("Export JSON", response.status_code == 200)
    
    response = requests.get(
        f"{BASE_URL}/export/csv?collection=quick_test",
        headers=headers
    )
...
try:
    response = requests.post(
        ...
        headers=headers,
        json={"type": "full", "description": "Quick test backup"}
    )
...
try:
    response = requests.post(
        ...
        headers=headers,
        json={"name": "Quick Test Org", "description": "Test"}
    )
...
try:
    response = requests.get(
        f"{BASE_URL}/relationships?collection=quick_test",
        headers=headers
    )
...
if failed == 0:
    ...
    print("  ✅ WoosCloud Storage is HEALTHY!")
    print("🎉"*20)
    
    print("\n  ✨ Verified Components:")
    print("  ✅ Storage API (CRUD)")
